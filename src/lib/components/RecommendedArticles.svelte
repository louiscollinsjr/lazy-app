...
{:else if error}
  <div class="flex items-center justify-center h-72 w-full bg-red-100 rounded-xl">
...
{:else if articles.length > 0}
  <section class="px-0 sm:px-0 lg:px-0 pb-32">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-16 mb-24 w-full">
      {#each articles.slice(0, 2) as article (article.id)}
        <ArticleCard {article} variant="default" />
      {/each}
    </div>
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-16 w-full">
